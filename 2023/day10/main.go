...
var moveMap = map[string][2]Tuple{
	"S": {{1, 0}, {0, 1}},
	"|": {{0, -1}, {0, 1}},
	"-": {{-1, 0}, {1, 0}},
	"L": {{0, -1}, {1, 0}},
...
func part2(input string) {
	charMapped := part1cycle(input)
	for _, line := range charMapped {
		for _, c := range line {
			fmt.Printf("%6s", c)
		}
		fmt.Println()
	}
	mapped, start := getMapAndStart(input)
	mapped[start.Y][start.X] = "F"
	charMapped = part2solve(charMapped, mapped)
	for _, line := range charMapped {
		for _, c := range line {
			fmt.Printf("%6s", c)
		}
...
func part2solve(charMapped [][]string, mapped [][]string) [][]string {
	insideCount := 0
	for d := 0; d < len(charMapped)+len(charMapped[0])-1; d++ {
		inside := false
		for x := 0; x < len(charMapped[0]); x++ {
			y := d - x
			if y < 0 || y >= len(charMapped) {
				continue
			}
			if isInt(charMapped[y][x]) {
				c := mapped[y][x]
				if c == "F" || c == "J" {
					continue
				}
...
func part1cycle(input string) [][]string {
	mapped, position := getMapAndStart(input)
	charMapped := [][]string{}
	for i := 0; i < len(mapped); i++ {
		tmp := []string{}
		for j := 0; j < len(mapped[i]); j++ {
			tmp = append(tmp, ".")
		}
		charMapped = append(charMapped, tmp)
	}
	charMapped[position.Y][position.X] = "0"

	p1 := Tuple{position.X + moveMap["S"][0].X, position.Y + moveMap["S"][0].Y}

	iters := 1
	for {
		next1 := getNextPosition(mapped, charMapped, p1)
		// fmt.Println(p1, p2)

		charMapped[p1.Y][p1.X] = fmt.Sprint(iters)
		iters++
		if next1.X == 0 && next1.Y == 0 {
			break
		}
		p1 = next1
...
func part1(input string) [][]string {
	...
	p1 := Tuple{position.X + moveMap["S"][0].X, position.Y + moveMap["S"][0].Y}
	p2 := Tuple{position.X + moveMap["S"][1].X, position.Y + moveMap["S"][1].Y}

	iters := 1
	for {
		next1 := getNextPosition(mapped, charMapped, p1)
		next2 := getNextPosition(mapped, charMapped, p2)
		// fmt.Println(p1, p2)

		charMapped[p1.Y][p1.X] = fmt.Sprint(iters)
		charMapped[p2.Y][p2.X] = fmt.Sprint(iters)
		iters++
		if next1 == next2 {
			break
		}
...
func getNextPosition(mapped [][]string, charMapped [][]string, start Tuple) Tuple {
	char := mapped[start.Y][start.X]
	p1 := Tuple{start.X + moveMap[char][0].X, start.Y + moveMap[char][0].Y}
	p2 := Tuple{start.X + moveMap[char][1].X, start.Y + moveMap[char][1].Y}
	// fmt.Println(start, p1, p2)

	if !isInt(charMapped[p1.Y][p1.X]) {
		return p1
	}
	if !isInt(charMapped[p2.Y][p2.X]) {
		return p2
	}
	return Tuple{}
}
...
func areSequential(n1, n2 string) bool {
	return math.Abs(float64(getInt(n1))-float64(getInt(n2))) == 1
...
func getMapAndStart(input string) ([][]string, Tuple) {
	sMap := [][]string{}
	start := Tuple{}
...
var test2 = `7-LJ.FF7J7-F-F--F7-.LL|7.LFJ--7F.J.L--FF-F77-F7|7F-F-777.JJ7|.J--F7JF-7-7-F-F-J.F-FF--7..|.LFJ77.77|F7FJ-77|F77FFL|7.|-7.|.7JFF--7|.|7-F--77
