...
var test2 = `five3onelxjninenine45
...
func main() {
	...
	lines := strings.Split(test2, "\n")

	sum := 0
	for _, line := range lines {
		sum += 10*getFirstNum(line) + getLastNum(line)
		fmt.Println(sum)
	}
	fmt.Println(sum)
...
func getFirstNum(line string) int {
	for i := 0; i < len(line); i++ {
		if num := getNum(line, i); num != 0 {
			return num
		}
	}
	return 0
...
func getLastNum(line string) int {
...
func getNum(line string, index int) int {
	if i, err := strconv.ParseInt(string(line[index]), 10, 32); err == nil {
		return int(i)
	}
	for key, value := range stringToInt {
		if strings.Index(line[index:], key) == 0 {
			return value
		}
	}
	return 0
